...
- Every domain carries `_confidence`, five-state, ranked `document > sighted > stated > estimated > inferred`, so the panel can mark an estimate as an estimate and the professional knows which figures are hard. `document` means Finn read the artefact. `sighted` means the person was on the source and read it off. A sighted value satisfies a document floor only while no working upload path exists for that field; the capability flag lives in code, not the registry. Where upload does work, the path text offers the upload and a typed answer from the screen is recorded as sighted, not document. `"stated"` is the person knew it and said it, no document; `"estimated"` is no document exists, or it couldn't be reached; `"inferred"` is solely for `flags.hardship`, which is written from Finn's read, never from asking. The linter reports every field currently resting on sighted.
- Every array item carries `id`, generated by code at first write. Never positional, never derived from capture order, never reused after deletion. `linked_asset_id` and the reconciliation pass walk ids.
...
**Placement.** The home loan lives in the home domain, investment property loans live on their property, and `debts.items` is everything else including splits.
